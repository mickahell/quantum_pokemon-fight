...
def battle(me, him):
    ...
    while play:
        ...
        action_j1 = quantum_action(me.pokemon, him.pokemon, qc_type, backend_sim)
        if action_j1 == 0 or len(me.team) == 1:
            attack_j1 = quantum_attaq(me.pokemon, him.pokemon, qc_type, backend_sim)
        else:
            copy_team = []
            for i in me.team:
                if i != me.pokemon:
        ...
        if me.pokemon.status == 0 or him.pokemon.status == 0:
            if me.pokemon.status == 0:
                ...
                if len(me.team) > 0:
                    if len(me.team) > 1:
                        next_poke = quantum_switch(me.team, him.pokemon, qc_type, backend_sim)
                    else:
                        next_poke = 0
